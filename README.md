...
const params: Nimble.MapParams = { url: 'url' };
const response: Nimble.MapResponse = await client.map(params);
```
...
<!-- prettier-ignore -->
```ts
const response = await client.map({ url: 'url' }).catch(async (err) => {
...
const response = await client.map({ url: 'url' }).asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: response, response: raw } = await client.map({ url: 'url' }).withResponse();
