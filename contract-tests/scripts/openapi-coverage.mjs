...
const SPEC_PATH = process.env.OPENAPI_SPEC_PATH || resolve(__dirname, '..', 'spec', 'openapi.yaml');

let spec;
if (SPEC_PATH.startsWith('http://') || SPEC_PATH.startsWith('https://')) {
    spec = await fetch(SPEC_PATH).then((r) => r.json());
} else {
    const raw = readFileSync(SPEC_PATH, 'utf8');
    spec = SPEC_PATH.endsWith('.yaml') || SPEC_PATH.endsWith('.yml') ? parseYaml(raw) : JSON.parse(raw);
}
...
const TESTED_OPERATIONS = new Map([
    ...
    ['POST /v1/serp', 'serp.run'],
    ['POST /v1/serp/async', 'serp.runAsync'],
    ['POST /v1/serp/batch', 'serp.runBatch'],
    ['POST /v1/media', 'media.run'],
    ['POST /v1/media/async', 'media.runAsync'],
]);
