...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { NIMBLE_API_KEY: readEnv('NIMBLE_API_KEY') ?? client.apiKey ?? undefined, NIMBLE_BASE_URL: readEnv('NIMBLE_BASE_URL') ?? client.baseURL ?? undefined };
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@nimble-way/nimble-js';
