...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      "{ id: string; created_at: string; description: string; display_name: string; domain_expertise: string; effort: string; goals: { id: string; goal: string; order: number; }[]; icon: string; is_active: boolean; output_schema: object; sources: { allow?: { id: string; domains: string[]; order: number; title: string; }[]; avoid?: string; block?: { domains: string[]; title: string; order?: number; }[]; prioritize?: string; }; suggested_questions: { id: string; order: number; question: string; }[]; updated_at: string; use_case: 'research' | 'enrichment' | 'dataset_building'; account_id?: string; agent_name?: string; workspace_id?: string; workspace_name?: string; }[]",
    markdown:
