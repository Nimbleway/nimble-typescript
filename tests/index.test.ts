...
import { APIPromise } from '@nimble-way/nimble-js/core/api-promise';

import util from 'node:util';
import Nimble from '@nimble-way/nimble-js';
import { APIUserAbortError } from '@nimble-way/nimble-js';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['NIMBLE_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: Nimble) => {
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['NIMBLE_LOG'] = 'debug';
    const client = new Nimble({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['NIMBLE_LOG'] = 'not a log level';
    const client = new Nimble({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'NIMBLE_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Nimble({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['NIMBLE_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Nimble({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Nimble({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['NIMBLE_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Nimble({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Nimble({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Nimble({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Nimble({ apiKey: 'My API Key', fetch: testFetch });
