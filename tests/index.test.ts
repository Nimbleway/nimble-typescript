...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['NIMBLEWAY_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Nimbleway) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Nimbleway({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Nimbleway({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['NIMBLEWAY_LOG'] = 'debug';
      const client = new Nimbleway({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['NIMBLEWAY_LOG'] = 'not a log level';
      const client = new Nimbleway({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'NIMBLEWAY_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['NIMBLEWAY_LOG'] = 'debug';
      const client = new Nimbleway({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Nimbleway({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['NIMBLEWAY_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Nimbleway({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('empty env variable', () => {
      process.env['NIMBLEWAY_BASE_URL'] = ''; // empty
      const client = new Nimbleway({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://gateway.staging.webit.live');
    });

    test('blank env variable', () => {
      process.env['NIMBLEWAY_BASE_URL'] = '  '; // blank
      const client = new Nimbleway({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://gateway.staging.webit.live');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Nimbleway({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['NIMBLEWAY_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Nimbleway({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Nimbleway({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Nimbleway({ apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Nimbleway({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Nimbleway({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Nimbleway({ apiKey: 'My API Key', fetch: testFetch });
